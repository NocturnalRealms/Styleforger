    ...
    <script>
        ...
        document.querySelectorAll('.style-tile').forEach(tile => {
            tile.addEventListener('click', function() {
                ...
                // Initialize voting system for this style
                currentStyleId = styleData.id;
                currentIsNiji = <?= $is_niji ? 'true' : 'false' ?>;
                loadVoteStatus(styleData.id, currentIsNiji);
        ...
        function openStyleModalFromHistory(styleData, historyItem = null) {
            ...
            // Initialize voting system for this style
            currentStyleId = styleData.id;
            currentIsNiji = historyItem && historyItem.isNiji;
            loadVoteStatus(styleData.id, currentIsNiji);
        ...
        // Global voting system variables (will be set when modal opens)
        let currentStyleId = null;
        let currentIsNiji = false;

// Enhanced copyStyleCode function
function copyStyleCode() {
    ...
    navigator.clipboard.writeText(styleNumber).then(() => {
        ...
        if (copyIcon) {
            ...
            setTimeout(() => { copyIcon.className = originalClass; }, 1000);
        }
    });
}
...
async function loadVoteStatus(styleId, isNiji = false) {
    try {
        const response = await fetch(`vote_api.php?style_id=${styleId}&is_niji=${isNiji ? 1 : 0}`);
        const data = await response.json();
    ...
    document.querySelectorAll('.style-tile').forEach(tile => {
        ...
        if (styleData.id == styleId) {
            ...
            if (voteDisplay) {
                ...
                if (voteSpan) {
                    voteSpan.textContent = newCount;
                    voteDisplay.style.display = newCount > 0 ? 'flex' : 'none';
                }
            }
        }
    });
